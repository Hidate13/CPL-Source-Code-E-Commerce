...
<main>
	<div class="width-limiter" style="padding: 0rem 2rem;">
		...
		<div class="selection-wrapper">
			...
			<button onclick="window.location.href='products'">Product
				Management</button>
			<button onclick="window.location.href='categories'">Categories</button>
			<button class="default" onclick="window.location.href='add-product'">Add
		...
		<section
			style="background-color: var(--bgDark); border-radius: 5px; margin-bottom: 3rem; padding: 0.5rem;">
			<h3 class="form-heading">Add a Product</h3>

			<sf:form class="flex-col align-center" style="gap: 1rem;"
				method="post" action="add_product" modelAttribute="product"
				...
				<div class="input-group flex-wrap" style="gap: 1rem;">
					<label class="select-label">Category<select
						class="input-select" name="categoryString"
						style="margin-left: 5px;" required>
							<option value="" selected disabled hidden="true">Choose
								here</option>
				...
				<label class="input-group flex-col" style="margin: 1rem 0rem 2rem;">Initial
					Amount <sf:input type="number" required="true" autocomplete="off"
						name="stock" path="stock" style="width: 200px;"
						oninput="this.value = this.value.replace(/[^0-9.]/g, '');" />
				</label>

				<label class="input-group flex-col" style="margin: 1rem 0rem 2rem;">&dollar;
					Price<sf:input type="number" step="0.01" required="true"
						autocomplete="off" name="price" path="price" style="width: 200px;"
						oninput="this.value=parseInt(this.value.replace('.',''))/100" />
				</label>

				<div class="input-group flex-col">
					<label class="file-input">Upload a Picture (Preferrably
						1:1)</label> <input required type="file" name="fileImage" id="photo"
...
<span id="error-text" class="form-error"></span> <sf:form
	...
	<label class="input-group flex-col">Description<textarea
			...
			rows="3" name="description" path="description"></textarea>
	</label>
