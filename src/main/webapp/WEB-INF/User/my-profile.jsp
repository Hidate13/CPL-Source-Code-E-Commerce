	...
	<div class="width-limiter align-center flex-col"
		...
		<div class="profile-container">
			<div class="img-wrapper">
				<img alt="Product" src="assets/Profile.png" width="200px">
				<div>
					<button id="openEditProfile" class="action-btn edit btnAnimation"
						style="margin-bottom: 2rem; font-size: 1.0rem">Edit
						Profile</button>
			...
			<div class="profile-details">
				...
				<span class="profile-label"><i class="material-icons">phone</i>Mobile
					No.</span>

				<c:if test="${mobile eq null }">
					<p>User hasn't given a mobile number yet.</p>
				</c:if>
				<p>${mobile }</p>

				<span class="profile-label"><i class="material-icons">house</i>Address</span>
				<c:if test="${address eq null }">
					<p>User provided an address yet.</p>
				</c:if>
				<p>${address }</p>
			</div>
		</div>
		<sec:authorize access="hasAnyRole('User')">
			<h2 class="section-heading hFont text-center"
				style="margin-top: 3rem; color: red;">Order History</h2>

			<div class="item-container"
				style="width: clamp(250px, 80vw, 500px) !important;">
...
<span id="error-text" class="form-error"></span> <sf:form
	id="editProfileForm" class="align-center flex-col form" method="post"
	action="update_profile" modelAttribute="user"
	...
	<label class="input-group flex-col">Full Name <input
		...
		autocomplete="off" onkeyup="validateFullname()" name="fullname"
		path="fullname" value="${fullname }" maxlength="50" />
	</label>

	<label class="input-group flex-col">Email <input id="email"
		type="email" required="true" placeholder="example@email.com"
		autocomplete="off" onkeyup="validateEmail()" name="email" path="email"
	...
	<label class="input-group flex-col">Address<textarea
			required="required" placeholder="Your physical address" rows="3"
			name="address" path="address">${address }</textarea>
	</label>
