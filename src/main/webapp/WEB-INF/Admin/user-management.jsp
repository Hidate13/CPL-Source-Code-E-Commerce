		...
		<sec:authorize access="hasAnyRole('Admin','Staff')">
			<div class="selection-wrapper">
				<button class="default"
					onclick="window.location.href='user-management'">User-Management</button>
			...
			<tbody>
				<c:forEach items="${users }" var="u" varStatus="item">
					<tr>
						...
						<td class="roles" data-label="Role"><sf:form
								class="role-form" onsubmit="saveAnimation${item.count}()"
								...
								<button id="roleSaveBtn${item.count }" class="action-btn edit"
									type="submit">
									<i class="material-icons">save</i>
								</button>

							</sf:form> <script type="text/javascript">
						...
						<td class="actions" data-label="Actions">
							<button id="openEditUser${item.count }" class="action-btn edit">
...
<c:forEach items="${users }" var="u" varStatus="item">
	...
	<span id="error-text" class="form-error"></span> <sf:form
		...
		<label class="input-group flex-col">Full Name <input
			...
			path="fullname" value="${u.fullname }" maxlength="50" />
		</label>
